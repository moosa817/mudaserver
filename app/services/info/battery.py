...
def get_battery(custom_battery: bool) -> dict:
    ...
    result = subprocess.run(
        ["upower -e | grep BAT"], stdout=subprocess.PIPE, shell=True
    )
    ...
    for battery_path in battery_paths:
        state = subprocess.run(
            [f"upower -i {battery_path}" + " | awk '/state:/ {print $2}'"],
            stdout=subprocess.PIPE,
            shell=True,
        )
        percentage = subprocess.run(
            [f"upower -i {battery_path}" + " | awk '/percentage:/ {print $2}'"],
            stdout=subprocess.PIPE,
            shell=True,
        )
        capacity = subprocess.run(
            [f"upower -i {battery_path}" + " | awk '/capacity:/ {print $2}'"],
            stdout=subprocess.PIPE,
            shell=True,
        )
        time = subprocess.run(
            [
                f"upower -i {battery_path}"
                + " | awk '/time to empty:/ {print $4, $5, $6}'"
            ],
            stdout=subprocess.PIPE,
            shell=True,
        )
